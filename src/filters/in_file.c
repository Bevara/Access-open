...
/* Progressive/streaming INPUT support: wraps a growing, JS-fed byte buffer
 * as a GF_FileIO source (protocol scheme "gfio://", already supported by
 * this filter - see fin_register()'s "file,isobmf,gmem,gfio" schemes list
 * below), so GPAC's demuxers can start processing a source file before it
 * has finished downloading. This is the input-side mirror of this project's
 * existing progressive OUTPUT support (see solver/loader.js's
 * flushProgressive, which polls a growing OUTPUT file on the virtual FS):
 * here, loader.js streams the source via fetch()'s ReadableStream and pushes
 * each chunk in as it arrives, instead of blocking on response.arrayBuffer()
 * for the whole download to complete before GPAC starts processing.
 *
 * webstream_create() is called once from JS to set up the buffer and obtain
 * a "gfio://..." URL to pass as the "-i" source argument. webstream_push()
 * is called once per received chunk. webstream_done() is called once the
 * download completes (or fails/is aborted), unblocking gfio_eof for any
 * remaining unread trailing bytes. webstream_destroy() releases everything
 * once the session no longer needs the source (mirrors gf_fileio_del).
 */
...
typedef struct
{
	...
	u8 *buffer;
	u64 alloc_size;
	u64 write_pos;   /* total bytes received so far */
	u64 read_pos;    /* current read cursor */
	u64 total_size;  /* expected total size - 0 until known (webstream_done or a Content-Length hint) */
	Bool done;       /* GF_TRUE once JS signals the download is complete (or aborted) */
} GF_WebStreamIO;

static GF_FileIO *webstream_fio_open(GF_FileIO *ref, const char *url, const char *mode, GF_Err *out_error)
{
	*out_error = GF_OK;
	/* the FileIO object is created eagerly by webstream_create() (not
	 * lazily via mode "url"), so every open just reuses that same ref -
	 * there is only ever one underlying buffer per streamed source */
	return ref;
}

static GF_Err webstream_fio_seek(GF_FileIO *fio, u64 offset, s32 whence)
{
	GF_WebStreamIO *ctx = (GF_WebStreamIO *) gf_fileio_get_udta(fio);
	/* gfio_seek_proc declares offset as u64, but the real caller
	 * (gf_fseek, see os_file.c) takes a signed s64 and calls gfio->seek()
	 * through that differently-typed pointer without conversion - so a
	 * negative SEEK_CUR/SEEK_END offset (e.g. demuxers seeking backward
	 * from EOF to read a trailing index/duration, as oggdmx does) arrives
	 * here as its raw bit pattern, not a huge unsigned value. Reinterpret
	 * as signed to match. */
	s64 soffset = (s64) offset;
	u64 new_pos;
	if (whence == SEEK_CUR) new_pos = ctx->read_pos + soffset;
	else if (whence == SEEK_END) new_pos = ctx->write_pos + soffset;
	else new_pos = (u64) soffset;
	/* Deliberately bounded to write_pos, not the (possibly already-known)
	 * total_size: some demuxers' duration/index scans assume seek+read
	 * are always synchronously satisfiable (true for a real local file)
	 * and spin retrying in a tight loop rather than yielding back to the
	 * scheduler - if a seek past write_pos were allowed to succeed while
	 * data for that region hasn't actually arrived yet, such a scan can
	 * hang the whole session forever waiting for bytes JS never gets a
	 * turn to deliver (confirmed by direct testing: relaxing this bound,
	 * even with total_size correctly seeded up front from
	 * Content-Length, reproducibly hung the session on an Ogg source).
	 * Rejecting the seek instead makes those callers fall back to
	 * whatever heuristic they use when duration can't be determined -
	 * safe, if less precise, for a source that isn't a plain file. */
	if (new_pos > ctx->write_pos) return GF_IO_ERR;
	ctx->read_pos = new_pos;
	return GF_OK;
}

static u32 webstream_fio_read(GF_FileIO *fio, u8 *buffer, u32 bytes)
{
	GF_WebStreamIO *ctx = (GF_WebStreamIO *) gf_fileio_get_udta(fio);
	u64 avail = (ctx->read_pos < ctx->write_pos) ? (ctx->write_pos - ctx->read_pos) : 0;
	u32 to_read = (u32) ((avail < (u64) bytes) ? avail : bytes);
	if (to_read) {
		memcpy(buffer, ctx->buffer + ctx->read_pos, to_read);
		ctx->read_pos += to_read;
	}
	return to_read;
}

static s64 webstream_fio_tell(GF_FileIO *fio)
{
	GF_WebStreamIO *ctx = (GF_WebStreamIO *) gf_fileio_get_udta(fio);
	return (s64) ctx->read_pos;
}

static Bool webstream_fio_eof(GF_FileIO *fio)
{
	GF_WebStreamIO *ctx = (GF_WebStreamIO *) gf_fileio_get_udta(fio);
	/* not EOF just because we've read everything received so far - only
	 * once JS has confirmed no more data is coming (webstream_done) */
	return (ctx->done && (ctx->read_pos >= ctx->write_pos)) ? GF_TRUE : GF_FALSE;
}

GF_EXPORT
GF_WebStreamIO *webstream_create(char *url)
{
	GF_WebStreamIO *ctx;
	GF_SAFEALLOC(ctx, GF_WebStreamIO);
	if (!ctx) return NULL;
	ctx->gfio = gf_fileio_new(url, ctx, webstream_fio_open, webstream_fio_seek, webstream_fio_read, NULL, webstream_fio_tell, webstream_fio_eof, NULL);
	if (!ctx->gfio) {
		gf_free(ctx);
		return NULL;
	}
...
GF_EXPORT
const char *webstream_get_url(GF_WebStreamIO *ctx)
{
	if (!ctx) return NULL;
	return gf_fileio_url(ctx->gfio);
}

GF_EXPORT
void webstream_push(GF_WebStreamIO *ctx, const u8 *data, u32 size)
{
	if (!ctx || !size) return;
	if (ctx->write_pos + size > ctx->alloc_size) {
		u64 new_alloc = ctx->alloc_size ? ctx->alloc_size * 2 : (1<<20);
		while (new_alloc < ctx->write_pos + size) new_alloc *= 2;
		ctx->buffer = gf_realloc(ctx->buffer, (size_t) new_alloc);
		ctx->alloc_size = new_alloc;
	}
	memcpy(ctx->buffer + ctx->write_pos, data, size);
	ctx->write_pos += size;
	gf_fileio_set_stats(ctx->gfio, ctx->write_pos, ctx->total_size, GF_FILEIO_CACHE_IN_PROGRESS, 0);
}

GF_EXPORT
void webstream_done(GF_WebStreamIO *ctx, u64 total_size)
{
	if (!ctx) return;
	ctx->total_size = total_size ? total_size : ctx->write_pos;
	ctx->done = GF_TRUE;
	gf_fileio_set_stats(ctx->gfio, ctx->write_pos, ctx->total_size, GF_FILEIO_CACHE_DONE, 0);
}

GF_EXPORT
void webstream_destroy(GF_WebStreamIO *ctx)
{
	if (!ctx) return;
	gf_fileio_del(ctx->gfio);
