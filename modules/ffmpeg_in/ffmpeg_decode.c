	...
	/*std MPEG-4 visual*/
		...
		if (esd->decoderConfig->objectTypeIndication == GPAC_OTI_VIDEO_AVC) {
			if (esd->decoderConfig->decoderSpecificInfo && esd->decoderConfig->decoderSpecificInfo->data) {
				...
				return (is_svc || esd->decoderConfig->rvc_config || esd->decoderConfig->bvr_config || esd->decoderConfig->predefined_rvc_config || esd->decoderConfig->predefined_bvr_config) ? GF_CODEC_MAYBE_SUPPORTED : GF_CODEC_SUPPORTED;
			}
			if (esd->decoderConfig->rvc_config || esd->decoderConfig->predefined_rvc_config || esd->decoderConfig->bvr_config || esd->decoderConfig->predefined_bvr_config || esd->has_ref_base) return GF_CODEC_MAYBE_SUPPORTED;
